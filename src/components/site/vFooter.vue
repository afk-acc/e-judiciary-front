<template>
  <div class="bg-footer_back py-16 mt-16 ">
    <div
      class="max-w-container mx-auto flex pb-4 flex-wrap px-4 maxlg:flex-wrap"
    >
      <div
        class="lg:w-5/12 maxlg:w-full lg:border-r maxlg:border-b maxlg:pb-4 border-l_gray_2 mr-4 mb-3"
      >
        <div class="font-bold text-l_white text-3xl">
          <router-link class="" :to="{ name: 'index' }">
            <svg class="w-[150px]" viewBox="0 0 512 512" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <p class="text-l_gray_2 text-xl mt-4 mb-8">
          We ara a lorem ipsum dolor sit amet, consectetur adipiscing elit, sed
          do eiusmod tempor incididun.
        </p>
        <div class="flex justify-center maxsm:flex-wrap gap-5">
          <div class="flex items-center">
            <div
              class="mr-4 rounded-full bg-l_gray_2 w-16 h-16 flex items-center justify-center"
            >
              <img src="/img/call-calling.svg" alt="" />
            </div>
            <div class="flex flex-wrap">
              <p class="text-xs w-full text-l_gray_2">{{ $t(`Есть вопросы?`) }}</p>
              <p class="text-l_white">310-437-2766</p>
            </div>
          </div>
          <div class="flex items-center">
            <div
      ...
      <div class="lg:w-5/12 maxxl:w-full maxlg:py-4">
        <p class="text-l_white text-2xl">{{ $t('Обратная связь') }}</p>
        <p class="text-l_gray text-lg mt-4 mb-10">
          {{
            $t(`Введите свой электронный адрес и отправьте его и наши специалисты свяжутся с вами в течении 25 минут.`)
          }}
        </p>
        <div class="flex maxsm:flex-wrap maxsm:justify-center maxsm:gap-5">
          <input
            type="text"
            :placeholder="this.$t('Введите свой E-mail')"
            class="bg-l_input_bg py-5 px-2 rounded-3xl w-9/12 text-l_white"
          />
          <vButton class="rounded-3xl">{{ $t('Отправить') }}</vButton>
        </div>
      </div>
      <div
        class="w-full border-t border-l_gray_2 text-l_gray text-lg mt-2 pt-2"
      >
        © 2000-2021, {{ $t('Все права защищены') }}
      </div>
    </div>
  </div>
</template>
